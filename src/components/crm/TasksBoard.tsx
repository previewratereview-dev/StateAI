...
function TaskRow({
  ...
  const pCfg = PRIORITY_CFG[task.priority as TaskPriority] || { label: task.priority, color: "#818286", bg: "transparent" };
  const sCfg = STATUS_CFG[task.status as TaskStatus] || { label: task.status, color: "#818286", bg: "transparent" };
      ...
      <div style={{ flex: isMobile ? 1 : "unset" }}>
        ...
        {isMobile && (
          <div style={{ display: "flex", gap: 8, marginTop: 8, alignItems: "center" }}>
            ...
            <div style={{ fontSize: "0.72rem", color: due?.isOverdue ? "#ef4444" : "#5d5e60" }}>
              ...
              {(["open", "in_progress", "done"] as TaskStatus[]).map(s => {
                ...
                return (
                  <button key={s} onClick={e => { e.stopPropagation(); onStatusChange(task.id, s); }}
                    disabled={isActive}
                    style={{ padding: "5px 14px", borderRadius: 999, fontSize: "0.72rem", fontWeight: 600, border: `1px solid ${isActive ? c.color + "50" : "rgba(177,178,180,0.12)"}`, background: isActive ? c.bg : "transparent", color: isActive ? c.color : "#818286", cursor: isActive ? "default" : "pointer", fontFamily: "inherit" }}>
                    {c.label}
                  </button>
                );
              })}
...
function AddTaskDrawer({ contacts, deals, onClose, onSaved }: { contacts: any[]; deals: any[]; onClose: () => void; onSaved: (t: Task) => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex" }}>
      ...
      <div style={{
        width: isMobile ? "100%" : 460, background: "#0d0d12", borderLeft: "1px solid rgba(177,178,180,0.1)",
  ...
  return (
    <div style={{ padding: isMobile ? "1rem" : "2rem", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "0.75rem" }}>
        <div>
          <h1 style={{ fontSize: isMobile ? "1.25rem" : "1.5rem", fontWeight: 700, color: "#fcfcfe", margin: 0, letterSpacing: "-0.03em" }}>Tasks</h1>
      ...
      <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
        ...
        {!isMobile && <div style={{ width: 1, background: "rgba(177,178,180,0.1)" }} />}
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
