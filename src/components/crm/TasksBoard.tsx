...
function TaskRow({
  ...
  const pCfg = PRIORITY_CFG[task.priority as TaskPriority] || { label: task.priority, color: "#64748B", bg: "transparent" };
  const sCfg = STATUS_CFG[task.status as TaskStatus] || { label: task.status, color: "#64748B", bg: "transparent" };
  ...
  const innerContent = (
    <>
      ...
      <div style={{ flex: isMobile ? 1 : "unset" }}>
        <div style={{ fontSize: "0.875rem", fontWeight: 600, color: task.status === "done" ? "#94A3B8" : "#1E293B", textDecoration: task.status === "done" ? "line-through" : "none" }}>
          {task.title}
        </div>
        ...
        {isMobile && (
          <div style={{ display: "flex", gap: 8, marginTop: 8, alignItems: "center" }}>
            ...
            <div style={{ fontSize: "0.72rem", color: due?.isOverdue ? "#ef4444" : "#94A3B8" }}>
      ...
      {!isMobile && (
        <>
          ...
          {/* Due date */}
          <div style={{ fontSize: "0.78rem", color: due?.isOverdue ? "#ef4444" : "#94A3B8" }}>
            {due ? (due.isOverdue ? "⚠️ " : "📅 ") + due.formatted : "—"}
          </div>
        </>
      )}
  ...
  return (
    <div style={{ borderBottom: "1px solid #E2E8F0", transition: "background 0.15s" }}
      ...
      {expanded && (
        <div style={{ padding: isMobile ? "0 1rem 1rem 2.25rem" : "0 1.25rem 1.25rem 3.25rem", display: "flex", flexDirection: "column", gap: "0.875rem" }}>
          {task.description && (
            <div style={{ fontSize: "0.85rem", color: "#64748B", background: "#FAFBFD", padding: "0.75rem", borderRadius: 10, border: "1px solid #E2E8F0", lineHeight: 1.6 }}>
              {task.description}
            </div>
          )}
          {/* Status controls */}
          <div>
            <div style={{ fontSize: "0.7rem", color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600, marginBottom: 8 }}>Update Status</div>
              ...
              {(["open", "in_progress", "done"] as TaskStatus[]).map(s => {
                ...
                return (
                  <button key={s} onClick={e => { e.stopPropagation(); onStatusChange(task.id, s); }}
                    disabled={isActive}
                    style={{ padding: "5px 14px", borderRadius: 999, fontSize: "0.72rem", fontWeight: 600, border: `1px solid ${isActive ? c.color + "50" : "#E2E8F0"}`, background: isActive ? c.bg : "transparent", color: isActive ? c.color : "#64748B", cursor: isActive ? "default" : "pointer", fontFamily: "inherit" }}>
                    {c.label}
                  </button>
                );
              })}
...
function AddTaskDrawer({ contacts, deals, onClose, onSaved }: { contacts: any[]; deals: any[]; onClose: () => void; onSaved: (t: Task) => void }) {
  ...
  const labelStyle: React.CSSProperties = { display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#64748B", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex" }}>
      ...
      <div style={{
        width: isMobile ? "100%" : 460, background: "#FFFFFF", borderLeft: "1px solid #E2E8F0",
        height: "100%", overflowY: "auto", padding: "1.75rem",
        boxShadow: "-20px 0 60px rgba(0,0,0,0.5)",
      }} className="crm-drawer-panel">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
          <h2 style={{ fontSize: "1.1rem", fontWeight: 700, color: "#1E293B", margin: 0 }}>New Task</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
            <div><label style={labelStyle}>Priority</label>
              <select name="priority" style={{ ...inputStyle, cursor: "pointer", background: "#FFFFFF", color: "#1E293B" }}>
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            ...
            <select name="deal_id" style={{ ...inputStyle, cursor: "pointer", background: "#FFFFFF", color: "#1E293B" }}>
              <option value="">— None —</option>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button type="submit" disabled={isPending} style={{ flex: 2, padding: "0.7rem", borderRadius: 10, background: "linear-gradient(135deg, #3B82F6, #6366F1)", border: "none", color: "#FFFFFF", cursor: isPending ? "not-allowed" : "pointer", fontFamily: "inherit", fontWeight: 700 }}>{isPending ? "Creating…" : "Create Task"}</button>
          </div>
        </form>
      </div>
    </div>
  ...
  return (
    <div style={{ padding: isMobile ? "1rem" : "2rem", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "0.75rem" }}>
        <div>
          <h1 style={{ fontSize: isMobile ? "1.25rem" : "1.5rem", fontWeight: 700, color: "#1E293B", margin: 0, letterSpacing: "-0.03em" }}>Tasks</h1>
          <div style={{ display: "flex", gap: 16, marginTop: 4, flexWrap: "wrap" }}>
            <span style={{ fontSize: "0.82rem", color: "#94A3B8" }}>{open} open</span>
        ...
        <button onClick={() => setShowAdd(true)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "0.65rem 1.25rem", borderRadius: 10, background: "linear-gradient(135deg, #3B82F6, #6366F1)", border: "none", color: "#FFFFFF", fontSize: "0.875rem", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 16px rgba(99,102,241,0.2)", minHeight: 44 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {(["all", "open", "in_progress", "done"] as const).map(s => (
            <button key={s} onClick={() => setStatusFilter(s as TaskStatus | "all")}
              style={{ padding: "0.5rem 0.875rem", borderRadius: 8, fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", fontFamily: "inherit", border: statusFilter === s ? "1px solid rgba(177,178,180,0.3)" : "1px solid transparent", background: statusFilter === s ? "#E2E8F0" : "transparent", color: statusFilter === s ? "#1E293B" : "#94A3B8" }}>
              {s === "all" ? "All" : STATUS_CFG[s as TaskStatus].label}
            </button>
          ))}
        </div>
        {!isMobile && <div style={{ width: 1, background: "#E2E8F0" }} />}
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {(["all", "urgent", "high", "medium", "low"] as const).map(p => {
            const cfg = p !== "all" ? PRIORITY_CFG[p as TaskPriority] : null;
            return (
              <button key={p} onClick={() => setPriorityFilter(p as TaskPriority | "all")}
                style={{ padding: "0.5rem 0.875rem", borderRadius: 8, fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", fontFamily: "inherit", border: priorityFilter === p ? `1px solid ${cfg ? cfg.color + "40" : "rgba(177,178,180,0.3)"}` : "1px solid transparent", background: priorityFilter === p ? (cfg ? cfg.bg : "#E2E8F0") : "transparent", color: priorityFilter === p ? (cfg ? cfg.color : "#1E293B") : "#94A3B8" }}>
      ...
      <div style={{ background: "#FFFFFF", backdropFilter: "blur(20px)", border: "1px solid #E2E8F0", borderRadius: 16, overflow: "hidden" }}>
        {!isMobile && (
          <div style={{ display: "grid", gridTemplateColumns: "20px 2fr 1fr 1fr 1.2fr auto", gap: "0.875rem", padding: "0.75rem 1.25rem", borderBottom: "1px solid #E2E8F0", background: "#F8FAFC" }}>
            {["", "Task", "Priority", "Status", "Due Date", ""].map((h, i) => <div key={i} style={{ fontSize: "0.7rem", fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</div>)}
          </div>
        )}
        {filtered.length === 0 ? (
          <div style={{ padding: "3rem", textAlign: "center", color: "#94A3B8" }}><div style={{ fontSize: "2rem", marginBottom: 8 }}>✅</div>No tasks. Create one!</div>
        ) : (
          filtered.map(task => (
