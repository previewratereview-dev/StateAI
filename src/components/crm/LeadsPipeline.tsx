...
function DealCard({ deal, isAdmin, onMove, onDelete }: { deal: Deal; isAdmin: boolean; onMove: (id: string, stage: DealStage) => void; onDelete: (id: string) => void }) {
  ...
  return (
    <div style={{ background: "rgb(13 13 18 / 80%)", border: "1px solid rgba(177,178,180,0.1)", borderRadius: 12, padding: "0.875rem", marginBottom: 8, boxShadow: "0 2px 12px rgba(0,0,0,0.3)", transition: "all 0.2s" }}
      onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(177,178,180,0.2)"}
      onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(177,178,180,0.1)"}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <div style={{ fontSize: "0.85rem", fontWeight: 600, color: "#fcfcfe", flex: 1, paddingRight: 8 }}>{deal.title}</div>
        <div style={{ position: "relative" }}>
          <button onClick={() => setShowMenu(s => !s)} style={{ background: "none", border: "none", color: "#5d5e60", cursor: "pointer", padding: "0 4px", fontSize: "1rem" }}>⋯</button>
          {showMenu && (
            <div style={{ position: "absolute", right: 0, top: "100%", background: "#0d0d12", border: "1px solid rgba(177,178,180,0.1)", borderRadius: 10, overflow: "hidden", zIndex: 10, minWidth: 140, boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
              ...
              {isAdmin && <button onClick={() => { setShowMenu(false); onDelete(deal.id); }} style={{ display: "block", width: "100%", padding: "0.6rem 0.875rem", textAlign: "left", background: "none", border: "none", color: "#ef4444", fontSize: "0.8rem", cursor: "pointer", borderTop: "1px solid rgba(177,178,180,0.06)", fontFamily: "inherit" }}>Delete</button>}
            </div>
          )}
        </div>
      </div>
      {contactName && <div style={{ fontSize: "0.75rem", color: "#818286", marginBottom: 6 }}>👤 {contactName}</div>}
      {deal.contacts?.company && <div style={{ fontSize: "0.72rem", color: "#5d5e60", marginBottom: 8 }}>🏢 {deal.contacts.company}</div>}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: "0.88rem", fontWeight: 700, color: stage.color }}>{formatCurrency(deal.value || 0)}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: "0.68rem", color: "#5d5e60" }}>{deal.probability}%</span>
...
function AddDealDrawer({ contacts, onClose, onSaved }: { contacts: { id: string; first_name: string; last_name: string }[]; onClose: () => void; onSaved: (d: Deal) => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex" }}>
      ...
      <div style={{ width: 420, background: "#0d0d12", borderLeft: "1px solid rgba(177,178,180,0.1)", height: "100%", overflowY: "auto", padding: "1.75rem", boxShadow: "-20px 0 60px rgba(0,0,0,0.5)" }}>
        ...
        <form onSubmit={handleSubmit}>
          {[
            ...
            <div key={f.name} style={{ marginBottom: "1rem" }}>
              <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#818286", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>{f.label}</label>
              <input name={f.name} type={f.type} required={f.required} style={inputStyle} />
            </div>
          ))}
          <div style={{ marginBottom: "1rem" }}>
            <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#818286", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Contact</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
            <div>
              <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#818286", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Value ($)</label>
              <input name="value" type="number" min="0" step="0.01" defaultValue="0" style={inputStyle} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#818286", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Probability (%)</label>
              <input name="probability" type="number" min="0" max="100" defaultValue="10" style={inputStyle} />
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
            <div>
              <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#818286", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Stage</label>
              <select name="stage" style={{ ...inputStyle, cursor: "pointer" }}>
                {STAGES.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
              </select>
            </div>
            <div>
              <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#818286", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Close Date</label>
              <input name="expected_close_date" type="date" style={inputStyle} />
            </div>
          </div>
          <div style={{ marginBottom: "1.5rem" }}>
            <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 600, color: "#818286", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Description</label>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button type="submit" disabled={isPending} style={{ flex: 2, padding: "0.7rem", borderRadius: 10, background: "linear-gradient(135deg, rgba(99,102,241,0.9), rgba(139,92,246,0.85))", border: "none", color: "#fcfcfe", cursor: isPending ? "not-allowed" : "pointer", fontFamily: "inherit", fontWeight: 700 }}>
              ...
              <div style={{ padding: "0.875rem", background: "rgb(13 13 18 / 70%)", backdropFilter: "blur(20px)", border: "1px solid rgba(177,178,180,0.08)", borderRadius: 14, marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: "0.82rem", fontWeight: 700, color: "#fcfcfe" }}>{stage.label}</span>
                  ...
                  <span style={{ fontSize: "0.72rem", color: "#5d5e60" }}>{formatCurrency(stageValue)}</span>
                </div>
              </div>
              {/* Cards */}
              <div style={{ flex: 1, overflowY: "auto" }}>
                {stageDeals.length === 0 && (
                  <div style={{ padding: "1.5rem", textAlign: "center", color: "#3d3e40", fontSize: "0.78rem", border: "1px dashed rgba(177,178,180,0.08)", borderRadius: 12 }}>No deals</div>
                )}
                {stageDeals.map(deal => (
