  ...
  return (
    <div style={{ padding: isMobile ? "1rem" : "2rem", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        <h1 style={{ fontSize: isMobile ? "1.25rem" : "1.6rem", fontWeight: 700, color: "#1E293B", margin: 0, letterSpacing: "-0.03em" }}>Reports & Analytics</h1>
        <p style={{ color: "#94A3B8", fontSize: "0.85rem", marginTop: 4 }}>Pipeline performance and team metrics</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
        {[
          ...
          <div key={kpi.label} style={{ ...card, padding: "1rem 1.25rem" }}>
            ...
            <div style={{ fontSize: "0.75rem", color: "#94A3B8", marginTop: 5, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>{kpi.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "1.5rem", marginBottom: "1.5rem" }}>
        {/* Pipeline by stage */}
        <div style={card}>
          <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "#1E293B", margin: "0 0 1.25rem", letterSpacing: "-0.01em" }}>Pipeline by Stage</h2>
                ...
                <div key={sg.stage}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ fontSize: "0.82rem", color: "#64748B", fontWeight: 500 }}>{cfg.label}</span>
                    </div>
        ...
        {/* Conversion funnel */}
        <div style={card}>
          <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "#1E293B", margin: "0 0 1.25rem", letterSpacing: "-0.01em" }}>Conversion Funnel</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ paddingTop: 12, borderTop: "1px solid #E2E8F0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "0.82rem", color: "#64748B" }}>Win Rate</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: "1.5rem" }}>
        {/* Contact breakdown */}
        <div style={card}>
          <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "#1E293B", margin: "0 0 1.25rem", letterSpacing: "-0.01em" }}>Contact Status</h2>
              ...
              return (
                <div key={cd.status}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                    <span style={{ fontSize: "0.8rem", color: "#64748B" }}>{cfg.label}</span>
        ...
        {/* Task completion */}
        <div style={card}>
          <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "#1E293B", margin: "0 0 1.25rem", letterSpacing: "-0.01em" }}>Task Overview</h2>
          {/* Donut-style */}
          <div style={{ display: "flex", justifyContent: "center", marginBottom: "1.25rem" }}>
            <div style={{ position: "relative", width: 100, height: 100 }}>
              ...
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                ...
                <div style={{ fontSize: "0.6rem", color: "#94A3B8" }}>done</div>
              </div>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[{ label: "Open", count: tasksOpen, color: "#f59e0b" }, { label: "In Progress", count: tasksInProgress, color: "#3b82f6" }, { label: "Done", count: tasksDone, color: "#10b981" }].map(t => (
              <div key={t.label} style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748B", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={card}>
          <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "#1E293B", margin: "0 0 1.25rem", letterSpacing: "-0.01em" }}>Bookings (7 days)</h2>
          {bookings.length === 0 ? (
            <p style={{ color: "#94A3B8", fontSize: "0.85rem" }}>No bookings in the last 7 days.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              <div style={{ paddingTop: 12, borderTop: "1px solid #E2E8F0", display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: "0.8rem", fontWeight: 700, color: "#1E293B" }}>{bookings.length}</span>
              </div>
            </div>
          )}
        </div>
